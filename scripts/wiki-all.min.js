...
function updateMeta(b){document.title=b.title;$("#title").text(b.title)}function onSaveSuccess(){updateMeta(client.meta)}function setDoc(b){index?(document.body.innerHTML=b.blob.markdown,b=document.createElement("script"),b.type="text/javascript",b.src="scripts/slides.js",b.onload=function(){handleDomLoaded()},document.body.appendChild(b)):(doc.editor.value=b.blob.markdown,onEditChange(),updateMeta(b))}
function getDoc(){if(!index)return{blob:{version:1,markdown:doc.editor.value},readers:["public"]}}function handleAppCache(){typeof applicationCache!="undefined"&&(applicationCache.status==applicationCache.UPDATEREADY?(applicationCache.swapCache(),location.reload()):applicationCache.addEventListener("updateready",handleAppCache,!1))};