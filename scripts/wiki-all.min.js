...
function onReady(){handleAppCache();doc=dom.bindIDs();client=new clientLib.Client(exports);client.saveInterval=0;client.addAppBar();$(doc.edit).click(toggleEditor);$(window).bind("scroll",onScroll);$.ajax({url:"slides.html",error:function(){console.log("ajax load error")},success:function(b){doc.output.innerHTML=b;renderedText=doc.editor.value=b;b=document.createElement("script");b.type="text/javascript";b.src="scripts/slides.js";b.onload=function(){handleDomLoaded()};document.body.appendChild(b);
