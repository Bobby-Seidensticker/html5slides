(function(a){function b(){}function d(a){if(!a)return 0;a=a.split(".");return 1E4*parseInt(a[0])+100*parseInt(a[1])+parseInt(a[2])}function e(a){for(var a=a.replace(/-/g,"_"),a=a.split("."),d=h,j=0;j<a.length;j++)d[a[j]]===void 0&&(d[a[j]]=new b),d=d[a[j]];return d}var h=a.namespace;if(h){if(d("3.0.1")<=d(h.VERSION))return;b=h.constructor}else a.namespace=h=new b;h.VERSION="3.0.1";a=b.prototype;a.module=function(a,b){var d=e(a);b&&b(d,e);return d};a.extend=function(a){for(var b in a)a.hasOwnProperty(b)&&
(this[b]=a[b])}})(this);
...
$(doc.output).html("<section class='slides'><article></article></section>");handleDomLoaded();handleLocationHash().doc||$.ajax({url:"slides.html",error:function(){console.log("ajax load error")},success:function(a){setDoc({title:"Editable HTML5 Slides",blob:{version:1.1,slides:a}})}});for(var a=$("script[type=slide-template]"),b,d=0;d<a.length;d++)b=a[d],stockCode[b.title]=$(b).text()}
...
function setDoc(a){console.log("setDoc()");a.blob.version===1?latestText=a.blob.markdown:a.blob.version===1.1?latestText=a.blob.slides:console.log("ERROR: unknown blob version number");index?($("body").html("<section class='slides'>"+latestText+"</section>"),handleDomLoaded()):(renderedText=doc.editor.value=latestText,$(doc.output).html("<section class='slides'>"+latestText+"</section>"),refresh(),getSlideBoundries(),setCursorPos(),onResize(),$(doc.output).css("display","block"),updateMeta(a))}
