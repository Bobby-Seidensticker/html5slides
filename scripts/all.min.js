...
var startTime=Math.floor((new Date).getTime()/1E3),client,doc,blob,latestText="",renderedText="",syncTime=5,editVisible=!1,editorInitialized=!1,editTimer,EDIT_BUFFER=1E3,currentScale,currentShift=0,index=!1,slideBoundries=[],curBoundriesText="",onReadyToggleEditor,BASE_HEIGHT=140,BUTTON_HEIGHT=38,IDEAL_WIDTH=1300,COMPRESSED_WIDTH=900,HEIGHT=700,OUTPUT_WIDTH=0.9,OUTPUT_WIDTH_EDIT=0.4317,stockCode={};function getDocid(){return handleLocationHash().doc}
function setDocid(a){handleLocationHash({doc:a})}
...
$(doc.output).html("<section class='slides'><article></article></section>");handleDomLoaded();handleLocationHash().doc||$.ajax({url:"slides.html",error:function(){console.log("ajax load error")},success:function(a){setDoc({title:"Editable HTML5 Slides",blob:{version:1.1,slides:a}})}});for(var a=$("script[type=slide-template]"),b,d=0;d<a.length;d++)b=a[d],stockCode[b.title]=$(b).text();onReadyToggleEditor=!0}
...
function setCrossTransform(a,b){var d="translate("+currentShift+"px, 0px) scale("+currentScale+")";$(a).css("-webkit-"+b,d);$(a).css("-moz-"+b,d);$(a).css("-o-"+b,d);$(a).css("-ms-"+b,d);$(a).css(b,d)}function updateMeta(a){document.title=a.title;$("#title").text(a.title);$(doc.fullscreen).unbind();$(doc.fullscreen).attr("href",location.href.replace("editor","").replace(/&page=[0-9]+/,"").replace("#doc=","docs/")+"/slides")}
